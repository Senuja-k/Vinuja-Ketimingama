...
import { Search, Home, Upload, Check, BadgeCheck, XCircle, Trash2, Edit3, Clock3, Shield, Cookie, Fingerprint, Settings, User, ChevronLeft, ChevronRight, DollarSign } from "lucide-react";
...
export default function AdminDashboard({ adminName = "Hirushan", onLogout, globalSettings, onSettingsUpdate }) {
    ...
    try {
      if (activeTab === "product_approval") {
        ...
        const data = res.data.data.map(o => ({
          ...
          customerName: o.customer ? ((o.customer.first_name || o.customer.username || "") + " " + (o.customer.last_name || "")).trim() : "Unknown",
        ...
        const data = res.data.data.map(item => ({
          ...
          paid_commission: item.paid_commission || 0,
          type: item.type,
  ...
  const handleBannerFileChange = (e) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        Swal.fire("Error", "Banner image size must be less than 5MB", "error");
        return;
      }
      setBannerFile(file);
  ...
  const handlePayMarketeer = async (e) => {
    ...
    try {
      ...
      formData.append('id', selectedMarketeer.id);
      formData.append('amount', parseFloat(payAmount));
      formData.append('isMarketeer', selectedMarketeer.isMarketeer ? 'true' : 'false');
      if (paymentSlip) {
        formData.append('slip', paymentSlip);
      }

      await api.post('/admin/marketeers/pay', formData, {
        headers: {
  ...
  const handleSlipUploadChange = (event) => {
    ...
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) {
      Swal.fire("Error", "Slip image size must be less than 5MB", "error");
      return;
    }
  ...
  return (
    <div className="container admin-dashboard-page" style={{ paddingTop: "90px", paddingBottom: "50px" }}>
      ...
      <div className="admin-header">
        ...
        {isMobile ? (
          <div className="admin-header-actions mobile-icon-actions">
            ...
            >
              <Home size={20} />
              {homePageCustomizationModalOpen && <span className="notification-dot"></span>}
            </button>
            <button
            ...
            >
              <Shield size={20} />
              {activeTab === "store_activation" && <span className="notification-dot"></span>}
            </button>
            <button
            ...
            >
              <User size={20} />
              {activeTab === "add_admin" && <span className="notification-dot"></span>}
            </button>
          </div>
        ) : (
      ...
      <div className="tabs-scroll-container">
        ...
        >
          Security & Cookies
          {activeTab === "security" && <span className="notification-dot"></span>}
        </button>
        <button
        ...
        >
          Global Switches
          {activeTab === "platform_settings" && <span className="notification-dot"></span>}
        </button>
        <button
        ...
        >
          Payment Methods
          {activeTab === "payment_methods" && <span className="notification-dot"></span>}
        </button>
        </div>
              ...
              <table className="admin-table">
                <thead>
                  <tr>
                    ...
                    {activeTab === "add_marketeers" && <th>Paid Comm.</th>}
                    <th>Details</th>
                    <th>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {(activeTab === "add_marketeers" ? filteredMarketeers : filteredAdmins).map((item, index) => {
                    ...
                    <tr key={index}>
                      ...
                      {activeTab === "add_marketeers" && <td>{item.paid_commission || 0}</td>}
                      ...
                      <td>
                        <div className={activeTab === "add_marketeers" ? "action-buttons action-buttons-icons" : "action-buttons"}>
                          {activeTab === "add_marketeers" && (
                            <button className="icon-button btn-pay" onClick={() => { setSelectedMarketeer(item); setPayMarketeerModalOpen(true); }} title="Pay">
                              <DollarSign size={18} />
                            </button>
                          )}
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if (file && file.size > 5 * 1024 * 1024) {
                          Swal.fire("Error", "Payment method icon size must be less than 5MB", "error");
                          return;
                        }
                        setPmFormData(prev => ({ ...prev, iconFile: file }));
                      }} 
                    />
                  </div>
                </div>
