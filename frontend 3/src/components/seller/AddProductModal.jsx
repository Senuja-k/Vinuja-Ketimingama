...
export default function AddProductModal({ isOpen, onClose, onSuccess, initialProduct = null, categories = [] }) {
    ...
    setVariantGroups(prev => prev.map((g, i) => {
      ...
      if (field === 'images') {
        const validFiles = value.filter(file => {
          if (file.size > 5 * 1024 * 1024) {
            Swal.fire("Error", `Image "${file.name}" size must be less than 5MB`, "error");
            return false;
          }
          return true;
        });
        newVariants[vIdx] = { ...newVariants[vIdx], images: [...newVariants[vIdx].images, ...validFiles] };
      } else {
  ...
  const handleSubmit = async () => {
    try {
      // ── STEP 1: Submit core product data (no images) ──────────────────────
      const metaData = new FormData();
      metaData.append('name', newProduct.name);
      metaData.append('description', newProduct.desc);
      metaData.append('category_id', newProduct.cat);
      metaData.append('marked_price', newProduct.price.toString().replace(/[^\d]/g, ''));
      if (newProduct.discount) metaData.append('discount_rate', newProduct.discount);
      metaData.append('delivery_fee', newProduct.deliveryFee);
      metaData.append('delivery_timeline', newProduct.deliveryTimeline);
      metaData.append('return_policy_days', newProduct.returnPolicy);

      const variantsForJson = variantGroups
        .filter(g => g.label.trim())
        .map(g => ({
          size: g.label,
          variants: g.variants.map(v => ({ color: v.color, qty: v.qty }))
        }));
      metaData.append('color', JSON.stringify(variantsForJson));
      ...
      metaData.append('quantity', totalQty);

      let productId;

      if (initialProduct) {
        metaData.append('_method', 'PUT');
        await api.post(`/seller/products/${initialProduct.id}`, metaData);
        productId = initialProduct.id;
      } else {
        const res = await api.post('/seller/products', metaData);
        productId = res.data.product.id;
      }

      // ── STEP 2: Upload variant images in per-variant batches ──────────────
      const variantImageBatches = [];

      variantGroups.forEach(g => {
        g.variants.forEach(v => {
          const newFiles     = v.images.filter(img => img instanceof File);
          const existingUrls = v.images.filter(img => typeof img === 'string');

          // Batch A: new files for this variant
          if (newFiles.length > 0) {
            const batch = new FormData();
            batch.append('_method', 'PUT');
            newFiles.forEach(file => {
              batch.append('images[]', file);
              batch.append('image_colors[]', v.color);
              batch.append('image_sizes[]', g.label);
            });
            variantImageBatches.push(batch);
          }

          // Batch B: existing image references for this variant
          if (existingUrls.length > 0) {
            const batch = new FormData();
            batch.append('_method', 'PUT');
            existingUrls.forEach(url => {
              batch.append('existing_images[]', url);
              batch.append('existing_image_colors[]', v.color);
              batch.append('existing_image_sizes[]', g.label);
            });
            variantImageBatches.push(batch);
          }
        });
      ...
      // Send each variant image batch sequentially
      for (const batch of variantImageBatches) {
        await api.post(`/seller/products/${productId}`, batch);
      }

      // ── STEP 3: Upload global images (no-variant products only) ───────────
      if (variantGroups.length === 0) {
        const globalBatch = new FormData();
        globalBatch.append('_method', 'PUT');
        let hasGlobalImages = false;

        [1, 2, 3, 4].forEach(num => {
          const img = newProduct[`image${num}`];
          if (img instanceof File) {
            globalBatch.append('images[]', img);
            hasGlobalImages = true;
          } else if (img && typeof img === 'string') {
            globalBatch.append('existing_images[]', img);
            hasGlobalImages = true;
          }
        });

        if (hasGlobalImages) {
          await api.post(`/seller/products/${productId}`, globalBatch);
        }
      }

      Swal.fire("Success", initialProduct ? "Product updated!" : "Product sent for approval!", "success");
      onSuccess();
      onClose();
  ...
  return (
    <div className="modal-overlay" style={{ zIndex: 10001 }}>
      <div className="post-details-modal-content" style={{ width: "850px", padding: "35px", maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        {/* Section 3: Size Chart & Variant Images */}
